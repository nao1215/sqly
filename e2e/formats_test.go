...
// TestSmoke_DocumentedCacheBehavior checks the claims the reference page makes
// about --cache: that a repeated run reuses the snapshot, that editing an input
// rebuilds it, and that an unreadable snapshot falls back to importing the
// sources rather than failing.
func TestSmoke_DocumentedCacheBehavior(t *testing.T) {
	dir := t.TempDir()
	input := writeFixture(t, dir, "events.csv", "id,kind\n1,click\n2,view\n")
	cachePath := filepath.Join(dir, "events.cache")
	args := []string{"--cache", cachePath, "--output-format", "csv",
		"--sql", "SELECT kind FROM events ORDER BY id", input}

	t.Run("the first run writes the snapshot at PATH", func(t *testing.T) {
		if _, stderr, code := run(t, "", args...); code != 0 {
			t.Fatalf("exit code = %d (stderr=%q)", code, stderr)
		}
		if !fileExists(cachePath) {
			t.Errorf("no snapshot at %s; the page says the first run builds it there", cachePath)
		}
	})

	t.Run("a later run answers the same", func(t *testing.T) {
		stdout, stderr, code := run(t, "", args...)
		if code != 0 {
			t.Fatalf("exit code = %d (stderr=%q)", code, stderr)
		}
		assertNoResidueDiagnostics(t, "warm run", stderr)
		records := parseCSVRecords(t, "warm run", stdout, ',')
		if len(records) != 3 || records[1][0] != "click" || records[2][0] != "view" {
			t.Errorf("warm run = %v, want click and view", records)
		}
	})

	t.Run("editing an input rebuilds the snapshot", func(t *testing.T) {
		writeFixture(t, dir, "events.csv", "id,kind\n1,click\n2,share\n")
		stdout, stderr, code := run(t, "", args...)
		if code != 0 {
			t.Fatalf("exit code = %d (stderr=%q)", code, stderr)
		}
		if !strings.Contains(stdout, "share") {
			t.Errorf("stdout = %q, want the edited row; the page says an edit rebuilds the cache", stdout)
		}
	})

	t.Run("an unreadable snapshot falls back instead of failing", func(t *testing.T) {
		writeFixture(t, dir, "events.cache", "not a sqlite database")
		stdout, stderr, code := run(t, "", args...)
		if code != 0 {
			t.Fatalf("a broken snapshot failed the run: exit %d (stderr=%q)", code, stderr)
		}
		if !strings.Contains(stdout, "share") {
			t.Errorf("stdout = %q, want the rows from the fallback import", stdout)
		}
		assertNoPanic(t, stdout, stderr)
	})
}
