...
**sqly** - мощный инструмент командной строки, который может выполнять SQL-запросы к CSV, TSV, LTSV, JSON и даже файлам Microsoft Excel™. sqly импортирует эти файлы в базу данных [SQLite3](https://www.sqlite.org/index.html) в памяти.
...
> [!WARNING]
> Поддержка JSON ограничена. Существует возможность прекращения поддержки JSON в будущем.
...
## Как использовать
sqly автоматически импортирует CSV/TSV/LTSV/JSON/Excel файлы в базу данных, когда вы передаете путь к файлу в качестве аргумента. Имя таблицы в базе данных совпадает с именем файла или листа (например, если вы импортируете user.csv, команда sqly создает таблицу user).

sqly автоматически определяет формат файла по расширению.
