...
## 使用方法
sqlyは、ファイルパスやディレクトリパスを引数として渡すと、CSV/TSV/LTSV/JSON/JSONL/Parquet/Excel/ACH/Fedwireファイル（圧縮版を含む）を自動的にDBにインポートします。同じコマンドでファイルとディレクトリを混在させることもできます。DBテーブル名は、ファイル名またはシート名と同じになります（例：user.csvをインポートした場合、sqlyコマンドはuserテーブルを作成します）。
...
ファイル名が数字で始まる場合、`sheet_` プレフィックスが付与されます（例：`2023-data.csv` はテーブル `sheet_2023_data` になります）。

### Excelシート名
Excelファイルをインポートする際、テーブル名は `ファイル名_シート名` の形式で作成されます。シート名もSQL互換性のためにサニタイズされます：
- スペース、ハイフン、ドットはアンダースコアに置き換えられます
- 非ASCII文字（`é`などのアクセント文字）は削除されます

例：
- ファイル `data.xlsx` のシート `A test` → テーブル `data_A_test`
- ファイル `report.xlsx` のシート `Café` → テーブル `report_Caf`

`--sheet`オプションを使用して、元のシート名（サニタイズ前）でシートを指定できます：
```shell
