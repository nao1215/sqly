...
func TestIsSupportedFile(t *testing.T) {
	...
	tests := []struct {
		...
		// CSV
		{"CSV file", "/test/path/to/sample.csv", true},
		{"Compressed CSV (.gz)", "test.csv.gz", true},
		{"Compressed CSV (.bz2)", "test.csv.bz2", true},
		{"Compressed CSV (.xz)", "test.csv.xz", true},
		{"Compressed CSV (.zst)", "test.csv.zst", true},
		...
		// TSV
		{"TSV file", "/test/path/to/sample.tsv", true},
		{"Compressed TSV (.gz)", "test.tsv.gz", true},
		// LTSV
		{"LTSV file", "/test/path/to/sample.ltsv", true},
		// JSON
		{"JSON file", "test.json", true},
		{"Compressed JSON (.gz)", "test.json.gz", true},
		// JSONL
		{"JSONL file", "test.jsonl", true},
		{"Compressed JSONL (.gz)", "test.jsonl.gz", true},
		// Parquet
		{"Parquet file", "test.parquet", true},
		{"Compressed Parquet (.gz)", "test.parquet.gz", true},
		// Excel
		{"XLSX file", "/test/path/to/sample.xlsx", true},
		{"Compressed XLSX (.gz)", "test.xlsx.gz", true},
		// Hidden files
		{"Hidden CSV file", "/test/path/to/.sample.csv", true},
		// Unsupported
		{"Text file", "/test/path/to/sample.txt", false},
		{"No extension", "/test/path/to/sample", false},
		{"Hidden file without extension", "/test/path/to/.sample", false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			if got := isSupportedFile(tt.filePath); got != tt.want {
...
func TestIsExcelFile(t *testing.T) {
	...
	tests := []struct {
		...
		{"Parquet file", "test.parquet", false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			if got := isExcelFile(tt.filePath); got != tt.want {
