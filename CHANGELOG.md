...
* A dot-command written wrong exits `2`, whichever command it is. Only `.import` did, and its own comment said it was keeping malformed input "in the usage class with every other 'you typed it wrong'" — which the other nine were not: `.header`, `.describe`, `.schema`, `.dump`, `.mode`, `.row-mismatch`, `.ls`, `.cd`, and `.save` all exited `1`, the code for a statement that ran and failed. Nothing ran in any of them. What a command does once its arguments are accepted is classified on its own, so `.cd` to a directory that is not there still exits `1`.
