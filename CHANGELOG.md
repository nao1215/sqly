...
* LTSV Values Lost The Whitespace Around Them: filesql's LTSV reader trimmed the value, while its LTSV writer wrote it and CSV kept its own, so `v:  padded  ` loaded as `padded`: a write-back through LTSV lost the spaces, and the same value read from an LTSV file and a CSV file disagreed. Requires filesql v0.29.0, which also fixes an XLSX export failing for a table whose name Excel cannot use as a sheet name — longer than 31 characters, or holding one of `: \\ / ? * [ ]`, which a name derived from an ordinary file name often is.
