...
* A ";" now ends a statement only where SQLite ends one. The shell asked whether the buffer ended with ";", so it submitted a fragment for every ";" inside a string literal (`SELECT 'a;`), inside a quoted identifier, or inside a trigger body — `CREATE TRIGGER ... BEGIN ... END;` could not be typed at all — and it refused to submit a finished statement carrying a trailing comment (`SELECT 1; -- note`), leaving the prompt waiting for nothing. The same scripts always ran through `--sql-file`, which uses the scanner the interactive shell now uses.
