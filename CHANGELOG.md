...
* Join Semantics E2E: the atago suite adds `join.atago.yaml`, covering join shapes the earlier suites did not: a LEFT JOIN keeping an unmatched row as NULL, a CROSS JOIN's cartesian product, a self-join, and a three-way join with aggregation. `helpers.atago.yaml` also gains a `.ls` scenario for the directory listing command.
